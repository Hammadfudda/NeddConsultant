      ...
      <nav className="mx-auto flex h-24 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-7 md:flex">
          <NavLink
            to="/"
            className={({ isActive }) =>
              `text-sm font-medium ${isActive ? 'text-sky-400' : 'text-slate-300 hover:text-white'}`
            }
          >
            Home
          </NavLink>
          ...
          <div className="group relative">
            ...
            <div className="invisible absolute left-1/2 top-full mt-3 w-72 -translate-x-1/2 rounded-2xl border border-white/10 bg-slate-950 p-2 opacity-0 shadow-2xl transition-all group-hover:visible group-hover:opacity-100">
              ...
              <Link
                ...
                className="mt-1 block rounded-xl bg-blue-600/10 px-4 py-3 text-sm font-semibold text-sky-400 hover:bg-blue-600/20"
              >
        ...
        <Link
          to="/contact"
          className="hidden items-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white hover:bg-blue-500 md:inline-flex"
        >
        ...
        <div className="border-t border-white/10 bg-slate-950 px-4 pb-5 pt-3 md:hidden">
          <div className="space-y-1">
            ...
            <button
              ...
              className="w-full rounded-xl px-4 py-3 text-left text-sm font-medium text-sky-400"
            >
              Service pages
            </button>
              ...
              <div className="ml-3 border-l border-white/10 pl-3">
                {SERVICE_ITEMS.map((service) => (
                  <Link
                    key={service.slug}
                    to={`/services/${service.slug}`}
                    className="block px-3 py-2 text-sm text-slate-400"
                  >
                    {service.title}
                  </Link>
                ))}
