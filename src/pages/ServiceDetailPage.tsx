  ...
  if (!service) {
    return (
      <div className="min-h-screen bg-[#07111f] px-4 pb-24 pt-40 text-center text-white">
        ...
        <Link to="/services" className="mt-6 inline-flex text-sky-400">
          Back to Services
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#07111f] pt-20 text-white">
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 sm:py-24 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:px-8">
          <div>
            ...
            <p className="mt-8 text-xs font-semibold uppercase tracking-[0.2em] text-sky-400">
              {service.eyebrow}
            </p>
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
          <div className="rounded-3xl border border-blue-400/10 bg-blue-400/5 p-7 sm:p-9">
          ...
          <div>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              {service.deliverables.map((item) => (
                <div
                  key={item}
                  className="flex gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-4 text-sm text-slate-300"
                >
          ...
          <div>
            ...
            <div className="mt-7 space-y-3">
              {service.outcomes.map((item) => (
                <div
                  key={item}
                  className="border-b border-white/10 pb-4 text-sm text-slate-300"
                >
                  {item}
                </div>
              ))}
